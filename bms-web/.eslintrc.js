...
module.exports = {
  ...
  rules: {
    ...
    'no-debugger': "off", // 打包时禁止debugger
    /**
     * @description: 自定义，"off"或者0（关闭规则关闭），"warn"或者1（在打开的规则作为警告（不影响退出代码）），"error"或者2（把规则作为一个错误（退出代码触发时为1））
     * @param {*}
     * @return {*}
     */
    ...
    'no-alert': process.env.NODE_ENV === 'production' ? 2 : 0, // 打包时禁止alert/comfirm/prompt
    'no-class-assign': 2,
    'no-cond-assign': 2,
    'no-const-assign': 2,
    'no-control-regex': 0,
